            ...
            while($k<count($j)){
                ...
                if(count($_SESSION["A0M"])==1){
                    if($_SESSION["private"]==1){
                        $sql2="INSERT INTO `record` (id, time, `groups`, tagged, A0, A1, A2, A3, A4d, A4, A5, A6, A7, A8, A9, B, private) VALUES (:v1, CURTIME(), 0, 0, :A0, :A1, :A2, :A3, :A4d, :A4, :A5, :A6, :A7, :A8, :A9, :B, 1)";
                        ...
                        $stmt->bindParam(":A9", $_SESSION["A9"]);
                        $stmt->bindParam(":B",  $B);
                        $stmt->execute();   //2.單一對象互動(groups=0)+自己(tagged=0)+公開(private=1)
                        
                        $sql3="INSERT INTO `record` (id, time, `groups`, tagged, A0, A1, A2, A3, A4d, A4, A5, A6, private) VALUES(:v1, CURTIME(), 0, 1, :A0, :A1, :A2, :A3, :A4d, :A4, :A5, :A6, 1)";
                        ...
                        $stmt->bindParam(":A5", $_SESSION["A5"]);
                        $stmt->bindParam(":A6", $_SESSION["A6"]);
                        $stmt->execute();   //3.單一對象互動(groups=0)+對方(tagged=1)+公開(private=1)
                                                
                    }else{
                        $sql4="INSERT INTO `record` (id, time, `groups`, tagged, A0, A1, A2, A3, A4d, A4, A5, A6, A7, A8, A9, B, private) VALUES (:v1, CURTIME(), 0, 0, :A0, :A1, :A2, :A3, :A4d, :A4, :A5, :A6, :A7, :A8, :A9, :B, 0)";
                        ...
                        $stmt->bindParam(":A9", $_SESSION["A9"]);
                        $stmt->bindParam(":B",  $B);
                        $stmt->execute();   //4.單一對象互動(groups=0)+自己(tagged=0)+不公開(private=0)   
                    }                    
                }else{
                    if($_SESSION["private"]==1){
                        $sql5="INSERT INTO `record` (id, time, `groups`, tagged, A0, A1, A2, A3, A4d, A4, A5, A6, A7, A8, A9, B, private) VALUES (:v1, CURTIME(), 1, 0, :A0, :A1, :A2, :A3, :A4d, :A4, :A5, :A6, :A7, :A8, :A9, :B, 1)";
                        ...
                        $stmt->bindParam(":A9", $_SESSION["A9"]);
                        $stmt->bindParam(":B",  $B);
                        $stmt->execute();   //5.多對象(團體)互動(groups=1)+自己(tagged=0)+公開(private=1)
                    
                        $sql6="INSERT INTO `record` (id, time, `groups`, tagged, A0, A1, A2, A3, A4d, A4, A5, A6, private) VALUES(:v1, CURTIME(), 1, 1, :A0, :A1, :A2, :A3, :A4d, :A4, :A5, :A6, 1)";
                        ...
                        $stmt->bindParam(":A5", $_SESSION["A5"]);
                        $stmt->bindParam(":A6", $_SESSION["A6"]);
                        $stmt->execute();   //6.多對象(團體)互動(groups=1)+對方(tagged=1)+公開(private=1)
                                               
                    }else{
                        $sql7="INSERT INTO `record` (id, time, `groups`, tagged, A0, A1, A2, A3, A4d, A4, A5, A6, A7, A8, A9, B, private) VALUES (:v1, CURTIME(), 1, 0, :A0, :A1, :A2, :A3, :A4d, :A4, :A5, :A6, :A7, :A8, :A9, :B, 0)";
                        ...
                        $stmt->bindParam(":A9", $_SESSION["A9"]);
                        $stmt->bindParam(":B",  $B);
                        $stmt->execute();   //7.多對象(團體)互動(groups=1)+自己(tagged=0)+不公開(private=0)         
                    }              
                }
                $k++;
            }                
...
<body>
    ...
    <div class="modal-content" style="border: none">
        <h5 style="text-align: right"><b>互動日期：<?=$_SESSION["A4d"]?></b></h5>
    </div>
    ...
    <div class="modal-content">
        ...
        <div class="modal-content" style="width: 100%; margin: 10px auto">
            <div class="modal-header">
                <div class="modal-title"><b>B. 這次互動後，<u>我認為</u><?=$r["id"][-2]?><?=$r["id"][-1]?>號 <?=$r["name"]?> 的心情：</b></div>
            </div>
            <div class="modal-body">
                <fieldset data-role="controlgroup" data-type="horizontal">
                    <label><input type="radio" name="<?=$r["id"]?>B" value="很不好">很不好</label>
                    <label><input type="radio" name="<?=$r["id"]?>B" value="有點不好">有點不好</label>
                    <label><input type="radio" name="<?=$r["id"]?>B" value="有點好">有點好</label>
                    <label><input type="radio" name="<?=$r["id"]?>B" value="很好">很好</label>
