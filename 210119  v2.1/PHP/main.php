<?php
    ...
    $sql6="SELECT *, name FROM `record` LEFT JOIN `account` ON record.A0=account.id WHERE record.id= :v1 and (A7 IS NULL or A8 IS NULL or B IS NULL) ORDER BY A4d desc, A4 desc";
    $stmt=$db->prepare($sql6);
    $stmt->bindParam(":v1", $id);
    $stmt->execute();                     //6.不完整的接觸紀錄
    $rs6=$stmt->fetchAll(PDO::FETCH_ASSOC);

    $sql7="SELECT *, name FROM `record` LEFT JOIN `account` ON record.A0=account.id WHERE record.id= :v1 and (A7 IS NOT NULL and A8 IS NOT NULL and B IS NOT NULL) ORDER BY A4d desc, A4 desc";
    $stmt=$db->prepare($sql7);
    $stmt->bindParam(":v1", $id);
    $stmt->execute();                     //7.完整的接觸紀錄
    $rs7=$stmt->fetchAll(PDO::FETCH_ASSOC);

    $sql8="SELECT COUNT(*) FROM `record` WHERE id= :v1 and MONTH(A4d)= :v2 and A7 IS NOT NULL and A8 IS NOT NULL and B IS NOT NULL";
    ...
    $sql9="SELECT A4d FROM `record` WHERE id= :v1 and MONTH(A4d)= :v2 and A7 IS NOT NULL and A8 IS NOT NULL and B IS NOT NULL GROUP BY A4d";
    ...
    if(isset($_POST["cdate"])){
        ...
        if($cdate==""){
            ...
            $sql12="SELECT *, name FROM `record` LEFT JOIN `account` ON record.A0=account.id WHERE record.id= :v1 and (A7 IS NULL or A8 IS NULL or B IS NULL) and A4d= :v2 ORDER BY A4 desc";
            ...
            $sql13="SELECT *, name FROM `record` LEFT JOIN `account` ON record.A0=account.id WHERE record.id= :v1 and (A7 IS NOT NULL and A8 IS NOT NULL and B IS NOT NULL) and A4d= :v2 ORDER BY A4 desc";
            ...
            $sql14="SELECT COUNT(*) FROM `record` WHERE id= :v1 and A4d= :v2 and A7 IS NOT NULL and A8 IS NOT NULL and B IS NOT NULL";
    ...
    <div style="text-align: center">
        ...
        <div class="calendar">
            ...
            <div style="color: red; font-size: 0.95em; font-weight: bold">
                <?php if(count($result)>=15){?>加油！距離有效會員只差 <?=20-count($result)?> 天！
                <?php }else if(count($result)>=20){?>恭喜！你已達本月有效會員標準！<?php }?>
            </div>
        </div>   
    </div>
