<?php
    ...
    $sql2="SELECT COUNT(*) FROM `record` WHERE id= :v1 and MONTH(time)= :v2 and A7 IS NOT NULL and A8 IS NOT NULL and A9 IS NOT NULL and B1 IS NOT NULL and B2 IS NOT NULL";
    $stmt=$db->prepare($sql2);
    $stmt->bindParam(":v1", $id);
    $stmt->bindParam(":v2", $month);
    $stmt->execute();                     //2.本月累計接觸筆數(考量被標註，A7~B2都需檢查是否完整)
    $rs2=$stmt->fetch(PDO::FETCH_ASSOC);

    $sql3="SELECT DATE(time) FROM `record` WHERE id= :v1 and MONTH(time)= :v2 and A7 IS NOT NULL and A8 IS NOT NULL and A9 IS NOT NULL and B1 IS NOT NULL and B2 IS NOT NULL GROUP BY DATE(time)";
    $stmt=$db->prepare($sql3);
    $stmt->bindParam(":v1", $id);
    $stmt->bindParam(":v2", $month);
    $stmt->execute();                     //3.新增接觸紀錄之日期(考量被標註，A7~B2都需檢查是否完整)
